...
def add_fpn(model, fpn_level_info):
	...
	#
	# Step 1: thinner feature by asy
	#param : model, thin_input, thin_output, kernel, dim_input, dim_output
	kernel_for_thin = [7,5,3,3]
	print("start")
	...
	if cfg.FPN.USE_GN:
		...
		c = model.ConvGN(
			thin_output_blobs[0],
			fpn_output_blobs[0],  # note: this is a prefix
		...
		fpn_output_blobs[0] = c  # rename it
	else:
		model.Conv(
			thin_output_blobs[0],
			fpn_output_blobs[0],
	...
	for i in range(num_backbone_stages - 1):
		add_topdown_lateral_module(
			model,
			fpn_output_blobs[i],         # top-down blob
			thin_output_blobs[i + 1],  # lateral blob
			fpn_output_blobs[i + 1],     # next output blob
			fpn_dim,                     # output dimension
			fpn_dim      # lateral input dimension
		)

	model.Copy(fpn_output_blobs[-1], dpa_output_blobs[-1])
	for i in range(num_backbone_stages - 1, 0, -1):
        	add_bottomup_lateral_module(
	            model,
	            dpa_output_blobs[i],        # bottom-up blob
	            fpn_output_blobs[i - 1],  	# lateral blob
	            dpa_output_blobs[i - 1],    # next output blob
	            fpn_dim,                    # output dimension
	            fpn_dim       				# input dimension (same as FPN)
	        )
    	for i in range(num_backbone_stages):
	    	add_wider_featuremap_module(
			model,
        		dpa_output_blobs[i],        # input blob
			wid_output_blobs[i],  		# output blob
			fpn_level_info.blobs[i],				# fpn_level_info
		        fpn_dim,                    # output dimension
	     	 	fpn_dim       				# input dimension (same as FPN)
		)
...
def add_thin_map_module(
    model, thin_input, thin_output, kernel, dim_input, dim_output
):
	"""Add a thin lateral module."""
	# Lateral 1x1 conv
	if cfg.FPN.USE_GN:
		# use GroupNorm
		lat = model.ConvGN(
		...
		mid_right = model.Conv(
			thin_input,
			thin_output + '_mid_right',
			dim_in=dim_input,
			dim_out=dim_output,
			kernel=[1, kernel],
			pad=pad_num,
		...
		out_right = model.Conv(
			mid_right,
			thin_output + '_out_right',
			dim_in=dim_output,
			dim_out=dim_output,
			kernel=[kernel, 1],
			pad=0,
	...
	model.net.Sum([out_left, out_right], thin_output)
...
def add_bottomup_lateral_module(
    model, dap_bottom, dap_lateral, dap_top, dim_top, dim_lateral
):
    ...
    # Sum lateral and top-down
    model.net.Sum([bu, dap_lateral], dap_top)
	
def add_wider_featuremap_module(
    model, wider_input, wider_output, fpn_level_info_blob, dim_in, dim_out
):
	xavier_fill = ('XavierFill', {})
	wid_blob_1 = model.Conv(
		wider_input,
		'wider1_{}'.format(fpn_level_info_blob),
		dim_in=dim_in,
		dim_out=dim_out,
		kernel=1,
		pad=0,
	...
	wid_blob_3_1 = model.Conv(
		wider_input,
		'wider3_1_{}'.format(fpn_level_info_blob),
		dim_in=dim_in,
		dim_out=dim_out,
		kernel=3,
		pad=1,
...
#	wid_blob_3_2 = model.Conv(
#		wid_blob_3_1,
#		'wider3_2_{}'.format(fpn_level_info_blob),
#		dim_in=dim_in,
#		dim_out=dim_out,
#		kernel=[3,1],
#		pad=0,
...
#		bias_init=const_fill(0.0)
#	)
	wid_blob_3_3 = model.Conv(
		wider_input,
		'wider3_3_{}'.format(fpn_level_info_blob),
		dim_in=dim_in,
		dim_out=dim_out,
		kernel=3,
		pad=1,
	...
	wid_blob_3_4 = model.Conv(
		wid_blob_3_3,
		'wider3_4_{}'.format(fpn_level_info_blob),
		dim_in=dim_in,
		dim_out=dim_out,
		kernel=3,
		pad=1,
	...
	concat_out,_ = model.net.Concat([wider_input, wid_blob_1, wid_blob_3_1, wid_blob_3_4],[wider_input+'_shuffled', 'concat_'+wider_input] , axis=1)
	model.Conv(
                concat_out,
                wider_output,
                dim_in=1024,
